...
void TFbxLoader::Init()
{
	...
	Manager = std::unique_ptr<FbxManager, TFbxDeleter>(FbxManager::Create());

	IOSettings = std::unique_ptr<FbxIOSettings, TFbxDeleter>(FbxIOSettings::Create(Manager.get(), IOSROOT));
...
bool TFbxLoader::LoadFBXMesh(std::string MeshName, TMesh& Mesh)
{
	//Get model path and convert to UTF8
	std::string ModelDir = TFileSystem::EngineDir() + "Resource/Models/";
	std::string ModelPath = ModelDir + MeshName;
	//char* ModelPath_UTF8;
	//FbxWCToUTF8(TFormatConvert::StrToWStr(ModelPath).c_str(), ModelPath_UTF8);

	// Create an importer 
	std::unique_ptr<FbxImporter, TFbxDeleter> Importer = std::unique_ptr<FbxImporter, TFbxDeleter>(FbxImporter::Create(Manager.get(), ""));
	...
	// Create a scene 
	std::unique_ptr<FbxScene, TFbxDeleter> Scene = std::unique_ptr<FbxScene, TFbxDeleter>(FbxScene::Create(Manager.get(), ""));
...
bool TFbxLoader::ImportMesh(FbxNode* Node, TMesh& Mesh)
{
	...
	for (uint32_t t = 0; t < numTriangles; t++)
	{
		for (uint32_t v = 0; v < 3; v++)
		{
			...
			//Add new index
			uint32_t index = uint32_t(Mesh.Indices32.size());
